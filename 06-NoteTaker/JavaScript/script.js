...
function getLocalStorage() {
  return localStorage.getItem("addedItemList")
    ? JSON.parse(localStorage.getItem("addedItemList"))
    : [];
}
...
function setUpItems() {
  ...
  if (storedItems.length > 0) {
    storedItems.forEach(function (item) {
      createAddedList(item.id, item.value);
    });
    addedItems.classList.add("showadded-items");
  }
}

function createAddedList(id, value) {
  //adding article
